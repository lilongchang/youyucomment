...
$("#showPdf").click(function () {
			...
			var ele = $("#tmpDiv");
            var targetDom = $("#repDetail");
            //把需要导出的pdf内容clone一份，这样对它进行转换、微调等操作时才不会影响原来界面
            var copyDom = targetDom.clone();
            //新的div宽高跟原来一样，高度设置成自适应，这样才能完整显示节点中的所有内容（比如说表格滚动条中的内容）
            copyDom.width(targetDom.width() + "px");
            //copyDom.height((parseInt(targetDom.height())) + "px");   //400是图片高度
            //copyDom.height((parseInt(targetDom.height())) + "px");   //400是图片高度####
            $('#tmpDiv').empty();
            $('#tmpDiv').append(copyDom);//ps:这里一定要先把copyDom append到body下，然后再进行后续的glyphicons2canvas处理，不然会导致图标为空
